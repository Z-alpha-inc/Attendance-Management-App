...
type MonthlyRes = { month: string; records: DayRecord[]; user?: { name: string; email: string } };
...
export default function AdminUserMonthlyPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await apiFetch<MonthlyRes>(`/api/admin/attendance?userId=${userId}&month=${month}`);
      setData(res);
    } catch (e: any) {
          ...
          <tbody>
            {(data?.records ?? []).map(r => (
              <tr key={r.date_key} className="border-t">
                ...
                <td className="px-3 py-2 text-right font-mono">{minutesToHHMM(r.workedMinutes ?? 0)}</td>
                <td className="px-3 py-2">
            ...
            {(!data?.records || data.records.length === 0) && (
              <tr><td colSpan={5} className="px-3 py-6 text-center text-gray-500">レコードがありません</td></tr>
            )}
          </tbody>
        </table>
      </div>
